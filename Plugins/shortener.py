...
class Shortener:
    ...
    def __init__(self, *, tokens: List[str], max_cache_size: int = config.DEFAULT_CACHE_SIZE):
        ...
        self._check_args()
        # Sorted for subsequent reproducible randomization.
        self._tokens = sorted(self._tokens)

        self._shorten_url = lru_cache(maxsize=self._max_cache_size)(
            self._shorten_url)  # type: ignore  # Instance level cache
        self._init_executor()
    ...
    def _cache_state(self) -> str:
        ...
        hit_percentage = ((100 * cache_info.hits) /
                          calls) if (calls != 0) else 0
        size_percentage = ((100 * cache_info.currsize) /
                           cache_info.maxsize) if cache_info.maxsize else 100
    ...
    def _check_args(self) -> None:
        ...
        if not (tokens and isinstance(tokens, list) and all(isinstance(token, str) for token in tokens) and (len(tokens) == len(set(tokens)))):
            ...
            raise exc.ArgsError(
                "Tokens must be a list of one or more unique strings.")
        ...
        if (not isinstance(max_cache_size, int)) or (max_cache_size < 0):
            raise exc.ArgsError(
                f"Max cache size must be an integer ≥0, but it is {max_cache_size}.")
    ...
    def _init_executor(self) -> None:
        self._max_workers = min(config.MAX_WORKERS, len(
            self._tokens) * config.MAX_WORKERS_PER_TOKEN)
        log.debug("Max number of worker threads is %s.", self._max_workers)
        self._thread_local = threading.local()
        # For conditional non-parallel execution.
    ...
    def _shorten_url(self, long_url: str) -> str:  # pylint: disable=too-many-locals,method-hidden
        ...
        if len(tokens) > 1:
            ...
            randomizer = random.Random(long_url)
            ...
            tokens = randomizer.sample(tokens, len(tokens))
        # Specified in reverse order due to pop().
        endpoints = config.API_URL_BITLINKS, config.API_URL_SHORTEN
        attempts = [(endpoint, token)
                    for endpoint in endpoints for token in tokens]
        ...
        while attempts:
            ...
            try:
                ...
                short_url_desc = f'with link {response_json["link"]}' if (
                    "link" in response_json) else "without link"
                log.debug(
                ...
                if isinstance(exception, (requests.Timeout, requests.ConnectionError)):
                    log.warning("Error receiving %s. %s",
                                response_desc, exc_desc)
                    ...
                    if response.status_code == 400 and response_json["message"] == "ALREADY_A_BITLY_LINK":
                        ...
                        # Returns normalized short URL.
                        return self._shorten_url(actual_long_url)
                    log.warning(
                        ...
                        # Still just a warning, and not an error yet.
                        "%s and text is %s. %s",
                        response_desc,
                        response.status_code,
                        response.text,
        ...
        log.debug("Returning short URL %s for long URL %s.",
                  short_url, long_url)
        return short_url
    ...
    def _test(self) -> None:
        ...
        log.debug("Tested API for long URL %s. Received short URL %s.",
                  long_url, short_url)
    ...
    def _usage(self, token: str) -> Dict[str, int]:
        ...
        response = session.get(
            config.API_URL_ORGANIZATIONS, headers=request_headers)
        orgs = response.json()["organizations"]
        for org in orgs:
            guid = org["guid"]
            response = session.get(config.API_URL_FORMAT_ORGANIZATION_LIMITS.format(
                organization_guid=guid), headers=request_headers)
    ...
    def usage(self) -> float:
        ...
        # 2nd check prevents bugs.
        if (num_tokens > 1) or not hasattr(self._thread_local, "session_get"):
        ...
        log.debug("%s retrieving usage for %s tokens%s.",
                  strategy_desc, num_tokens, resource_desc)
        start_time = time.monotonic()
        usages = list(mapper(self._usage, tokens))
        time_used = time.monotonic() - start_time
        usage = sum(u["used"] for u in usages) / sum(u["limit"]
                                                     for u in usages)
    ...
    def shorten_urls(self, long_urls: List[str]) -> List[str]:
        ...
        # 2nd check prevents bugs.
        if (len(set(long_urls)) > 1) or not hasattr(self._thread_local, "session_post"):
            strategy_desc = "Concurrently"
            ...
            resource_desc = ""
            mapper = map  # type: ignore
        log.debug("%s retrieving %s short URLs%s.",
                  strategy_desc, num_long_urls, resource_desc)
        start_time = time.monotonic()
